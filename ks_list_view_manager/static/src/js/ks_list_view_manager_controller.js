...
//              'click button.o_button_import': 'importdata',
        ...
        renderButtons: function ($node) {
            ...
            if (this.ks_lvm_mode && !this.$buttons.find("#ks_dropdown").length)
            {
                try
                {
                    var $lvmButtons = $(QWeb.render('ks_list_view_manager.buttons', {widget: this}));
                    $lvmButtons.appendTo($(".o_control_panel").find('.o_cp_bottom_left'));
                    $(document).ready(function(){
                        $(document).find('th[data-name=create_date]').trigger('click');
                        
                        var cron = setInterval(function()
                        {
                            var count = $(document).find('button.o_list').length;
                            if(count>0)
                            {
                                $(document).find('button.o_list').trigger('click');
                                clearInterval(cron);
                            }                            
                        },1);
                    });
